		...
		<ul>
			 <?php $featured = get_option('woo_featured_category'); $feature = new WP_Query('category_name='.$featured.'&amp;showposts=8'); while ($feature->have_posts()) : $feature->the_post(); $do_not_duplicate = $post->ID; $preview = get_post_meta($post->ID, 'preview', true); ?>
				<li class="popular">
                    ...
                    <a title="Permanent Link to <?php the_title(); ?>" href="<?php the_permalink() ?>" rel="lightbox"><img src="<?php echo bloginfo('template_url'); ?>/thumb.php?src=<?php echo get_post_meta($post->ID, "image", $single = true); ?>&amp;h=138&amp;w=216&amp;zc=1&amp;q=95" alt="<?php the_title(); ?>" class="post-preview left" /></a>   
                    <?php } ?>       	
