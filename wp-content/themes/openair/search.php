					...
					<?php if (have_posts()) : ?>
						...
						<li class="post clearfix">
							...
							<div class="post-content">
                                ...
                                <a title="Permanent Link to <?php the_title(); ?>" href="<?php the_permalink() ?>" rel="lightbox"><img src="<?php echo bloginfo('template_url'); ?>/thumb.php?src=<?php echo get_post_meta($post->ID, "image", $single = true); ?>&amp;h=<?php if ( get_option('woo_image_height') <> "" ) { echo get_option('woo_image_height'); } else { ?>145<?php } ?>&amp;w=<?php if ( get_option('woo_image_width') <> "" ) { echo get_option('woo_image_width'); } else { ?>218<?php } ?>&amp;zc=1&amp;q=95" alt="<?php the_title(); ?>" class="post-preview left" /></a>      
                                <?php } ?>    	
