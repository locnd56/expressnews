...
// Widget Functions (used for the Your Favourites Widgets)
require_once ($functions_path . '/widget-functions.php');
...
function woothemes_wp_head() { 
     ...
     if ($color != '') {
          ?> <link href="<?php bloginfo('template_directory'); ?>/styles/<?php echo $stylesheet; ?>/<?php echo $color; ?>.css" rel="stylesheet" type="text/css" /><?php 
     } else { 
		 $stylesheet2 = get_option('woo_alt_colours');
		 if($stylesheet2 != ''){
			?><link href="<?php bloginfo('template_directory'); ?>/<?php woo_style_path(); ?>/<?php echo $stylesheet2; ?>" rel="stylesheet" type="text/css" /><?php         
      	 }
     }
