...
function get_flickrRSS() {
	...
	// get the feeds
	if ($type == "user") { $rss_url = 'http://api.flickr.com/services/feeds/photos_public.gne?id=' . $userid . '&tags=' . $tags . '&format=rss_200'; }
...
function flickrRSS_subpanel() {
     if (isset($_POST['save_flickrrss_options'])) {
	...
	<div class="wrap">
		...
		<p><div class="submit"><input type="submit" name="save_flickrrss_options" value="<?php _e('Save Options', 'save_flickrrss_options') ?>"  style="font-weight:bold;" /></div></p>
        </form>       
    </div>
